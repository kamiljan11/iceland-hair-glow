    ...
    <section className="py-14 md:py-32 bg-background">
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center mb-8 md:mb-16">
          ...
          <p className="font-body text-muted-foreground text-sm md:text-lg max-w-md mx-auto">
            The perfect gift for someone special. Available in-salon or delivered beautifully wrapped.
          </p>
        </div>
        ...
        <div className="flex gap-3 overflow-x-auto pb-4 snap-x snap-mandatory -mx-4 px-4 md:mx-0 md:px-0 md:grid md:grid-cols-2 lg:grid-cols-4 md:gap-4 md:overflow-visible md:pb-0 md:snap-none scrollbar-none max-w-4xl md:mx-auto">
          {tiers.map((tier) => (
            <div
              key={tier.amount}
              className="flex-shrink-0 w-[55vw] sm:w-[40vw] md:w-auto snap-center group relative border border-border bg-card p-5 md:p-8 text-center hover:border-gold/40 transition-all duration-500 hover:shadow-xl hover:shadow-gold/5"
            >
              ...
              <p className="font-display text-lg md:text-2xl font-bold text-foreground mb-1.5 md:mb-2">
                {tier.amount}
              </p>
        ...
        <div className="text-center mt-6 md:mt-10">
          <a
            href="#booking"
            className="inline-flex items-center gap-2 bg-gold text-gold-foreground px-6 md:px-8 py-3.5 md:py-4 text-xs md:text-sm tracking-[0.2em] uppercase font-body font-semibold hover:bg-gold/90 transition-all duration-300 hover:shadow-lg hover:shadow-gold/20 touch-target"
          >
            <Gift size={14} />
            Purchase Gift Card
          </a>
          <p className="font-body text-muted-foreground text-[10px] md:text-xs mt-2.5 md:mt-3">
