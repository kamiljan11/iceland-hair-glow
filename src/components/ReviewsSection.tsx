    ...
    <section className="py-16 md:py-32 bg-secondary/30">
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center mb-10 md:mb-16">
          ...
          {/* Google trust badge */}
          <div className="inline-flex items-center gap-2 bg-card border border-border px-4 py-2">
            <div className="flex gap-0.5">
              {[...Array(5)].map((_, i) => (
                <Star key={i} size={14} className="text-gold fill-gold" />
              ))}
            </div>
            <span className="font-body text-sm text-foreground font-semibold">4.9</span>
            <span className="font-body text-xs text-muted-foreground">· 127 Google Reviews</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 max-w-6xl mx-auto">
          {reviews.map((review, i) => (
            <div
              key={i}
              className="bg-card border border-border p-6 md:p-8 hover:border-gold/30 transition-all duration-300"
            >
              ...
              <p className="font-body text-foreground/80 text-sm md:text-base leading-relaxed mb-4">
                "{review.text}"
              </p>
              <div className="border-t border-border pt-3 flex justify-between items-end">
                <div>
                  ...
                  <p className="font-body text-xs text-gold">{review.service}</p>
                </div>
